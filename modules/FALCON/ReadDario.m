...
%ReadDario Sets up the analysis model for the Intrinsic Strain-Based code.
%
% Inputs:
%   - 'aircraft_data' : A structure describing the aircraft geometry in
%   terms of nodes, elements and aerodynamic panels. Uses the data
%   structure that Dario developed. TODO - Change to my FEModel
%   representation.
%
%
...
%Grab associated 'node' and 'aero' data
...
for ii = 1 : nPart
    ...
    part_idx     = ismember(partNames, allPartIDs{ii});
...
for jj = 1 : nPart
    ...
    for ii = 1 : nElem
        idxNodeB = aircraft_data.Node.ID == partNodes(ii + 1);
        idxNodeA = aircraft_data.Node.ID == partNodes(ii);
        s_coord(:, ii) = nodeCoords(idxNodeB, :) - nodeCoords(idxNodeA, :);
    end
    %Overall length of this beam and the beam element coords
    %   - TODO : Get this from the beam model!
    l(jj) = sum(sqrt(sum(s_coord.^2, 1)));
    s{jj} = cumsum([0 sqrt(sum(s_coord.^2, 1))]);
    % Surface Type
    type{jj}   = 'LS';
    ...
    %% Beam s-coordinates
%     if 1%jj == 1 || jj == 2
%         s{jj}      = cumsum([0 sqrt(sum(s_coord.^2, 1))]);%[0 : l(jj)/nElem   : l(jj)];
%     else
%         s{jj}      = [0 : l(jj)/nElem   : l(jj)];
%     end
    clear s_coord
    s2{jj}     = sort([s{jj},s{jj}(2:end-1)-1e-5]);
    s_mp{jj}   = (s{jj}(1:end-1)+s{jj}(2:end))/2;%/4
    s_aero{jj} = [0 : l(jj)/nElem*2  : l(jj)];%*2*4/2/4
    ...
    RenderPoints{jj}.y_upper = zeros(nElem,2*(iaf.n+1));
    RenderPoints{jj}.y_lower = zeros(nElem,2*(iaf.n+1));
    RenderPoints{jj}.z_upper = zeros(nElem,2*(iaf.n+1));
    RenderPoints{jj}.z_lower = zeros(nElem,2*(iaf.n+1));
    ...
    f               = zeros((nElem+1)*6,1);
    f_a             = zeros((nElem+1)*6,1);
    f_mp_pnt        = zeros( nElem   *6,1);
    Matrices.f{jj}        =  f;
    Matrices.f_a{jj}      =  f_a;
