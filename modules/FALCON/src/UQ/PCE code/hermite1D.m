function poly = hermite1D(samples, order)

%Outputs 1D hermite polynomials at a number of sample points, for an input
...
%containing each of the hermite polynomials up to the required order,
...
for i = 1:D
    ...
    for j = 2:order
        % Recursive definition of the Hermite Polynomials
        currentPoly(:,j+1) = samples(:,i).*currentPoly(:,j)-(j-1)*currentPoly(:,j-1);
    end
