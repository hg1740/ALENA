...
numEm = 100000; % Number of emulations in surrogate model (arbitrarily large number)
...
std = [0.151867291/10/3];% 1.03e9 5.9e8 3.66e-6];% 0.03 1.4e9 4e-9] % standard deviations of variables
param = [mu;std]; % input required for sampling
% Observations are for input PDF plots which are included for illustrative
% purposes. Delete if not required.
observations = normrnd(repmat(mu,5000,1),repmat(std,5000,1));
...
switch distribution
    case 'normal'
        ...
        std = repmat(param(2,:),numSam,1);
        % Take inverse of normal CDF based on input mean and standard
        % deviation values
%         xSam = norminv(FxSam,mu,std);
        xSam = rho_MCS(1:numSam);
...
% observations = normrnd(repmat(mu,200,1),repmat(std,200,1));
...
[fy,y] = ksdensity(yEm);% Calcuates PCE PDF using kernal density estimate (hist may also be used)
%[fy y] = ksdensity(ySam);
