...
if ~isempty(find(diff(RBE2.ID)==0,1))
    ...
    for i = 1 : length(RBE2.ID)
        ...
        GDL = zeros(1,nGDL);
        for j = 1 : nGDL
            if ischar(RBE2.GDL(i).data(j))
                GDL(j) = str2double(RBE2.GDL(i).data(j));
            else
                GDL(j) = (RBE2.GDL(i).data(j));
            end
        end
        %
        RBE2.GDL(i).data = GDL;
...
if PARAM.SPC
    ...
    if isempty(spcindex)
        ...
        for n=1:ngrid
            if Node.ID(n)  && isempty(find(Node.ID(n)==IDS,1)) && ~isempty(find(Node.DOF(n, :),1))% structural node
                ...
                Node.DOF3(n, :) = int32(zeros(1,6));
                Node.DOF3(n, index) = int32(ndof+1 : ndof+length(find(index==1)));
            ...
            elseif Node.ID(n) && ~isempty(find(Node.DOF(n, :),1))
                ...
                Node.DOF3(n, :) = int32(zeros(1,6));
                Node.DOF3(n,setdiff(1:6,RBE2.GDL(IDRBE2(ids)).data)) = int32(ndof+1 : ndof+6-NGDL(ids));
    ...
    for n=1:ngrid
        if Node.ID(n) && isempty(find(Node.ID(n)==IDS,1)) && ~isempty(find(Node.DOF(n, :),1))% structural master node
            ...
            Node.DOF3(n, :) = int32(zeros(1,6));
            Node.DOF3(n, :) = int32(ndof+1 : ndof+6);
            ndof = ndof + 6;
        elseif Node.ID(n) && ~isempty(find(Node.DOF(n, :),1))
            ids = find(Node.ID(n)==IDS);
            Node.DOF2(n, :) = int32(zeros(1,6));
