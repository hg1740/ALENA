...
function K = st_lin_matrix(INFO, DOF, NODER, COORD, BAR, BEAM, CELAS, CBUSH)
...
[iel, jel, vel] = set_DT_mat([1:INFO.nbar], BAR, BAR.R, DOF, COORD, COORD, NODER, NODER);
...
if ~isempty(CELAS.ID)
  ...
  for k = 1 : length(CELAS.ID) 
    ...
    elseif CELAS.Node(k,2) == 0 % node 1 linked to ground
      ...
      vc = [ones(ndof1,1)*CELAS.K(k)*ndof2;ones(ndof2,1)*CELAS.K(k)*ndof1];
      icel = [icel;ic]; jcel = [jcel;ic]; vcel = [vcel;vc];
      for nce = 1 : ndof1
        ic = ones(ndof2,1)*double(CELAS.DOF(k,1).data(nce));
        jc = double(CELAS.DOF(k,2).data');
        vc = -ones(ndof2,1)*CELAS.K(k);
        icel = [icel;ic];
        jcel = [jcel;jc];
        vcel = [vcel;vc]; 
      end
...
if ~isempty(CBUSH.ID)
    ...
    for k = 1:length(CBUSH.ID)
        % I need to multiply the stiffness entries by a rotation matrix
        % that corresponds to the the coordinate frame i'm interested in!!!
        % diagonal term
        ic = [double(CBUSH.DOF(k,1).data');double(CBUSH.DOF(k,2).data')];
        jc = [double(CBUSH.DOF(k,1).data');double(CBUSH.DOF(k,2).data')];
        vc = [CBUSH.K(k).data';CBUSH.K(k).data'];
        ibus = [ibus;ic];
        jbus = [jbus;jc];
        vbus = [vbus;vc];
        
        % off-diagonal terms
        ic = double(CBUSH.DOF(k,1).data');
        jc = double(CBUSH.DOF(k,2).data');
        vc = -CBUSH.K(k).data';
        ibus = [ibus;ic];
        jbus = [jbus;jc];
        vbus = [vbus;vc];
        
        ic = double(CBUSH.DOF(k,2).data');
        jc = double(CBUSH.DOF(k,1).data');
        vc = -CBUSH.K(k).data';
        ibus = [ibus;ic];
        jbus = [jbus;jc];
        vbus = [vbus;vc];
