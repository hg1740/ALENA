...
if ntot >0
    ...
    if (~EL_CONTR)
        ...
        % Update the rotational terms so that the they are about the NEW
        % centre of gravity (added 25/4/17)
        J = J - MCG(1)*crossm(NEWCG - CG)*crossm(NEWCG - CG);
        ...
        for n=1:ntot
            ...
            for j=1:3
                ...
                if (ELEM.M(1,1,j,n) > 0.0)
                    ...
                    offset = (ROT(:,:,ELEM.Conn(n,j)) * offset')';
                    v = COORD(ELEM.Conn(n,j),1:3) - NEWCG; % Changed 15/04/17 to -ve
                    
                    crossm_v = crossm(v);
                    J = J + ROT(:,:,ELEM.Conn(n,j)) * ELEM.M(4:6,4:6,j,n) * ROT(:,:,ELEM.Conn(n,j))' - ...
                        ELEM.M(1,1,j,n) .* (crossm(v) * crossm(v)) + ...
                        2 * crossm_v * ROT(:,:,ELEM.Conn(n,j))' * ELEM.M(1:3,4:6,j,n)* ROT(:,:,ELEM.Conn(n,j));
                end
            end
        end
        ...
        NEWMRP(4:6, 1:3) = S;
        NEWMRP(1:3, 4:6) = S';
        NEWMRP(4:6,4:6)  = R'*NEWCGM(4:6,4:6)*R - mtot .* (crossm(v) * crossm(v));
