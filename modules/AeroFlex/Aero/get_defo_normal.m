...
for n = 1:np
    ...
    %
    %Z = cross(r1,r2);
    Z = [r1(2).*r2(3)-r1(3).*r2(2); r1(3).*r2(1)-r1(1).*r2(3); r1(1).*r2(2)-r1(2).*r2(1)];
    
    if span<0
      Z = -Z;
    end
    %Z = Z ./ norm(Z) ;
    Z = Z / (Z(1)^2 + Z(2)^2 + Z(3)^2)^0.5;
    
    %Y = cross(Z ,[1, 0 , 0]);
    Y = [0; Z(3); -Z(2)];
    
    %Y = Y ./ norm(Y);
    Y = Y / (Y(1)^2 + Y(2)^2 + Y(3)^2)^0.5;
    
    %X = cross(Y, Z);
    X = [Y(2).*Z(3)-Y(3).*Z(2); Y(3).*Z(1)-Y(1).*Z(3); Y(1).*Z(2)-Y(2).*Z(1)];

    %X = X ./ norm(X);
    X = X / (X(1)^2 + X(2)^2 + X(3)^2)^0.5;

    REF_P = [X(1) Y(1) Z(1); X(2) Y(2) Z(2); X(3) Y(3) Z(3)]; % local panel reference frame
% 	NDEFO(n, 1:3) = Z + (REF_P * DELTAN(n,:)')';
    NDEFO(n, 1:3) = Z;% + DELTAN(n,:);
    %NDEFO(n ,1:3) = NDEFO(n,1:3) ./ norm(NDEFO(n,1:3));
end
