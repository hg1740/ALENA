...
%
% File History:
%   - 09/10/2019 (C.Szczyglowski) 
%       Added preallocation for loop variables.
function [lattice] = defo_wakesetup(lattice, state, ref) 

infdist=6*ref.b_ref;

[a, b, ~] = size(lattice.VORTEX);
...
c=[2 7]; %Why only the 2nd and 7th indices???
nC = numel(c);
...
%Preallocate
%   - Edit : C.Szczyglowski 09/10/2019
dx   = zeros(a, nC);
dy   = zeros(a, nC);
dz   = zeros(a, nC);
INF1 = zeros(a, 1, 3);
...
% figure, hold on;
% plot3(lattice.VORTEX(:, :, 1), lattice.VORTEX(:, :, 2), lattice.VORTEX(:, :, 3), 'ro')
% plot3(V2(:, :, 1), V2(:, :, 2), V2(:, :, 3), 'gs')

lattice.VORTEX = [INF1(:,1,:) V2(:,:,:) INF2(:,1,:)];

% plot3(lattice.VORTEX(:, :, 1), lattice.VORTEX(:, :, 2), lattice.VORTEX(:, :, 3), 'b^')
