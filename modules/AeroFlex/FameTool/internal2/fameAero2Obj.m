...
%   Private function.
function obj = fameAero2Obj(obj,Part)

if strcmp(Part,'HTP')
    startNumber = obj.Opts.Struct.HTPCAEROID;
    nSpan       = obj.Opts.Aero.htp_nSpan;
    nChord      = obj.Opts.Aero.htp_nChord;
    PartName = ['Stbd',Part];
...
elseif strcmp(Part,'VTP')
    ...
    nSpan       = obj.Opts.Aero.vtp_nSpan;
    nChord      = obj.Opts.Aero.vtp_nChord;
    PartName = Part;
end
...
obj.Fame.Geometry.(Part).ref_point(1) = Outline(1);
% If it is the VTP we are looking at ..  swap the y for the z TEMPORARILY
if strcmp(Part,'VTP')
    NewOutline = Outline;
    NewOutline(:,2) = Outline(:,3);
    NewOutline(:,3) = Outline(:,2);
    Outline = NewOutline;
end

if Outline(1,3)~=0
    RootNode = [0,0,Outline(1,3)];
else
    RootNode = obj.Fame.Geometry.(Part).ref_point;
end
Spanin  = min(Outline(:,2));
Spanout = max(Outline(:,2));
Span    = Spanout-Spanin;
...
if ~strcmp(Part,'VTP')
    ...
    if Ndih ~= npoints - 1
        ...
        NewEta = uniquetol([Outline(:,2);Fame.Geometry.(Part).dihedral(:,1)],1e-5);
        ...
        Fame.Geometry.(Part).dihedral = [NewEta,NewDihedral];
    ...
    Outline = dummy;
    Outline(:,3) = Outline(:,3) - Outline(1,3);
end
...
for i = 1:length(CsIdx)
    Control_LE_beg = [Control_LE_beg;obj.Fame.Geometry.(Part).ControlSurfaces.(CsNames{CsIdx(i)}).eta_beg_le];
    Control_LE_end = [Control_LE_end;obj.Fame.Geometry.(Part).ControlSurfaces.(CsNames{CsIdx(i)}).eta_end_le];
end

CSEdges = uniquetol([Control_LE_beg;Control_LE_end;(Outline(:,2)-Outline(1,2))/Span],0.005);

npointsNew = length(CSEdges);

ControlOutline = [];
ControlOutline(1:npointsNew,1) = interp1((Outline(1:npoints,2)-Outline(1,2))/Span,Outline(1:npoints,1),CSEdges);
ControlOutline(1:npointsNew,2) = Span*CSEdges;
ControlOutline(1:npointsNew,3) = interp1((Outline(1:npoints,2)-Outline(1,2))/Span,Outline(1:npoints,3),CSEdges);

ControlOutline(npointsNew+1: 2*npointsNew,1) = interp1((Outline(npoints+1:2*npoints,2)-Outline(1,2))/Span,Outline(npoints+1:2*npoints,1),flipud(CSEdges));
...
for nCaero = 1:nSections
    ...
    CAERO.geo.sw(nCaero,1)     = 180 * atan((ControlOutline(nCaero + 1,1) + sweepLoc * (ControlOutline(chordIndexOut(2)) - ControlOutline(chordIndexOut(1)))...
    ...
    CAERO.geo.c(nCaero,1)      = ControlOutline(chordIndexIn(2),1) - ControlOutline(chordIndexIn(1),1);
    CAERO.geo.t(nCaero,1)      = (ControlOutline(chordIndexOut(2),1) - ControlOutline(chordIndexOut(1),1))...
        /(ControlOutline(chordIndexIn(2),1) - ControlOutline(chordIndexIn(1),1));
    CAERO.geo.b(nCaero,1)          = (ControlOutline(nCaero + 1,2) - ControlOutline(nCaero,2));
    
    CAERO.geo.rootAirfoil{nCaero,1} = '0012';
    CAERO.geo.tipAirfoil{nCaero,1}  = '0012';
    if strcmpi(Part,'vtp')
        CAERO.geo.dih(nCaero,1) = 90; % TODO added dihedral entry
    else
    ...
    CAERO.geo.fc(nCaero,:)          = [0,0];
    CAERO.geo.flapped(nCaero,1)     = 0;
    CAERO.geo.symetric(nCaero,1)    = 0;
    
    CAERO.geo.ny(nCaero,1)          = nyPanels(nCaero);
    CAERO.geo.nx(nCaero,1)          = nChord;
%     
%     
%     
...
% 
%     if strcmp(Part,'Wing')
%         if obj.Opts.Aero.wing_addTwist
...
%         CAERO.geo.rootAirfoil{ip(nCaero) + i,1} = '0012';
%         CAERO.geo.tipAirfoil{ip(nCaero) + i,1}  = '0012';
...
%         CAERO.geo.fnx(ip(nCaero) + i,1) = 0;
%         CAERO.geo.fc(ip(nCaero) + i,:)  = [0,0];
%         CAERO.geo.flapped(ip(nCaero) + i,1)  = 0;
%         CAERO.geo.symetric(ip(nCaero) + i,1)  = 0;
%     end
%     
...
if strcmp(Part,'VTP')
    ...
    NewOutline = ControlOutline;
    NewOutline(:,2) = ControlOutline(:,3);
    NewOutline(:,3) = ControlOutline(:,2);
    ControlOutline = NewOutline;
    
    startY = CAERO.geo.startZ;
    startZ = CAERO.geo.startY;
    CAERO.geo.startY = startY;
    CAERO.geo.startZ = startZ;
end
...
if strcmpi(Part,'vtp')
    ...
    obj.Fame.Geometry.(Part).chord(:,2) = [CAERO.geo.c;CAERO.geo.c(end)*CAERO.geo.t(end)];
...
for i = 1:numel(CAERO.id)
    ...
    CaeroObj.flapped     = CAERO.geo.flapped(i);
    CaeroObj.fnx         = CAERO.geo.fnx(i);
    CaeroObj.fc          = CAERO.geo.fc(i,:);
    CaeroObj.symetric    = CAERO.geo.symetric(i);
    CaeroObj.csType      = [];
    CaeroObj.csId        = [];
    CaeroObj.csData      = [];
    ...
    CaeroObj.le          = leKinks;
    CaeroObj.te          = teKinks;
    CaeroObj.part        = PartName;
    CaeroObj.csLink      = 1;

    if numel(obj.Mdl.Caero) == 0
