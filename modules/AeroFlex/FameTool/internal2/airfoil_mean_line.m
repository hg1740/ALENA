...
function [EP, Yu, Yl, xa, angle] = airfoil_mean_line(foil,resample,STEP,plotfoil)
...
switch TYPE
    ...
    case 2
        ...
        %% Extract the upper and lower coordinate points of the airfoil
        xup = A(2:Nu+1,1);
        yup = A(2:Nu+1,2);
        xdw = A(Nu+2:end,1);
        ydw = A(Nu+2:end,2);
        ...
        %% Determine if there are missing points
        if (Nu ~= Nl)
        ...
        %% Resample the coordinate points if requested by the user
        EP = Xl;
        ...
        %% Determine the mean line
        ml = 0.5.*(Yu+Yl);

        %% Determine the angle generated by the mean camber line
        for ll = 1:length(ml)
            if ll ==1
        ...
        %% Plot the airfoil if requested by the user
        if plotfoil == 1
            figure(100);
            plot(EP,Yu,'-b.');
            hold on
            ...
            plot(EP,ml,'-r.');
            plot(EP,angle,'-k.');
            h = legend('Upper Surface','Lower Surface','Mean Camber Line','Camber Angle');
            set(h,'Units','normalized','Position',[0.6387 0.1260 0.2571 0.1655]);
            axis equal
            axis tight
            hold off;
            xlabel('x');ylabel('y');
