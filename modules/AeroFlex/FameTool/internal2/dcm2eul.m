function [r1 r2 r3] = dcm2eul( dcm, varargin )
%  DCM2EUL Create rotation angles from direction cosine matrix.
%   [R1 R2 R3]  = DCM2EUL( N ) calculates the set of rotation angles,
...
%
%   [R1 R2 R3] = DCM2EUL( N, S ) calculates the set of rotation
...
%
%   [R1 R2 R3] = DCM2EUL( N, S, LIM ) calculates the set of rotation
...
%      [pitch, roll, yaw] = dcm2eul( dcm, 'YXZ' )
%
...
if any(~isreal(dcm) || ~isnumeric(dcm))
    error(message('aero:dcm2eul:isNotReal'));
end

if ((size(dcm,1) ~= 3) || (size(dcm,2) ~= 3))
    error(message('aero:dcm2eul:wrongDimension'));
end
...
if nargin == 1
    ...
    if ischar( varargin{1} )
        ...
        error(message('aero:dcm2eul:notChar'));
    end
    if nargin == 3
        if ischar( varargin{2} )
            lim = varargin{2};
        else
            error(message('aero:dcm2eul:notChar1'));
        end
    end
end
...
switch lower( type )
    ...
    otherwise
        error(message('aero:dcm2eul:unknownRotation', type));
end
