...
function I = assembly_midv_interp_mat(ncaero, NODE, AERO)
...
[~, nv, dim] = size(AERO.lattice.VORTEX);
[tot_np,~,~] = size(AERO.lattice.COLLOC);
switch nv
...
%             if beam_model.Aero.Interp.Temp~=nset
%                 beam_model.Aero.Interp.sol = [];
%                 beam_model.Aero.Interp.Temp = nset;
%             end
            ...
            switch AERO.Interp.Type(ninterp)
                ...
                case 2
                    ...
                    %Imv_check = rbf_interface(str_data, aer_data(PANEL+ntot, 1:3), weight, rmax, tcond);
                    Imv = rbf_interfaceD(str_data, aer_data(PANEL+ntot, 1:3), weight, rmax, tcond);
            end
