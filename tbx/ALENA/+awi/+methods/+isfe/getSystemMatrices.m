function SYS = getSystemMatrices(ModelData, System, AnalysisParam)
%SYS = getSystemMatrices(X, Matrices, Aero, Sim, AnalysisParam)
...
numElem = [ModelData.NumElem];
numNode = [ModelData.NumNode];
...
switch AnalysisParam.AnalysisType
    ...
    case 'static'
        ...
        %Preallocate
        SYS = initialiseStaticSysStructure(numNode, numElem);

        %Loop through parts and return the system matrices
        for jj = 1 : numel(ModelData)
            
            %Get the coordinates and orientations at each node
            [coords, CaB] = strains2coords(ModelData(jj));
            
            %Grab part data
            nElem   = numElem(jj);
            nNode   = numNode(jj);
            
            %Grab state vector for this body
            partInd = System.BodyLocalLoadIndex(1, jj) : System.BodyLocalLoadIndex(2, jj);
            x_f     = System.x_f(partInd);
            ...
            for ii = 1 : nElem %For each element... 
                ...
                indRow     = (1 : 12) + ((ii - 1) * 6);
                indCol     = (1 : 6)  + ((ii - 1) * 6);
                indMidNode = ModelData(jj).MatrixIndexing.NodeMidInd(ii);
                
                %Grab data
                complianceMatrix = ModelData(jj).C(:, :, ii);                   %Compliance matrix without shape function terms
                massMatrix       = ModelData(jj).M(:, :, ii);                   %Mass matrix without shape function terms
                e1               = ModelData(jj).E1(:, ii);                     %Orientation vector
                Ab               = ModelData.ShapeFunctions.Spatial(:, :, ii);  %Ab = (A_1)' * dS + (A_2)' * (dS^2 / 2)
                
                %Grab the element loads
                %   - X.x_f_pl is the vector of nodal/elemental forces
                elemLoads = x_f(indCol);
                                
                %% Structural terms
                %   - Calculates the stiffness matrix and the derivative of
                %     the system matrix w.r.t the states (beam loads) for
                %     this body.
                ...
                SYS(jj).Atot(indRow,indCol)      = SYS(jj).Atot(indRow,indCol)      + A;
                SYS(jj).dQstiftot(indRow,indCol) = SYS(jj).dQstiftot(indRow,indCol) + dQstif;
                ...
                %                 end
                CaB_e = CaB(:, :, indMidNode);  %Aircraft to element
                CGB_e = System.CGa * CaB_e;     %Global to element  
...
%                 end
                Ra    = coords(:, indMidNode) + ModelData(jj).P0;
                CaB_d = [CaB_e , zeros(3) ; zeros(3), CaB_e ];
                ...
                SYS(jj).Btot_r(:, indCol)  = Ra_t1 * CaB_d * ModelData.ds_node(ii);
                %             Btot_r_aero{jj}(:,ind3)  = Btot_r_grav{jj}(:,ind3);%Ra_t2 * CaB_d * (Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii));
                
                %% Distributed loads due to gravity
                if AnalysisParam.bModelGravity
                    %Calcualte gravity force in the local (element) and
                    %global frame
                    gravForce                 = massMatrix * [CGB_e', zeros(3) ; zeros(3), CGB_e'] * AnalysisParam.GravityVector;
                    SYS(jj).f_grav(indCol,1)      = gravForce;
                    SYS(jj).f_grav_glob(indCol,1) = [CGB_e' zeros(3); zeros(3) CGB_e']*gravForce;
                else
                    SYS(jj).f_grav(indCol,1)      = zeros(6,1);
                    SYS(jj).f_grav_glob(indCol,1) = zeros(6,1);
                end
                
            end
            
            for ii = 1 : nNode %For each node... 
                
                %Index numbers for this node
                indCol  = (1 : 6)  + (ii - 1) * 6;
                indNode = ModelData(jj).MatrixIndexing.NodeInd(ii);                
...
%                 else
%                     CaB_e = CaB(:, :, indNode);
%                     CGB_e = System.CGa * CaB_e;
%                     %Nodal positions
%                     Ra    = coords(:, indNode) + ModelData(jj).P0;
%                     %Nodal orientations
%                     CaB_d = [CaB_e, zeros(3) ; zeros(3), CaB_e];
%                 end
                ...
                SYS(jj).f_glob(indCol,1) = [CGB_e', zeros(3) ; zeros(3), CGB_e'] * ModelData(jj).f_a(indCol);
                
                Ra_t                   = [eye(3), zeros(3) ; skew(Ra), eye(3)];
                SYS(jj).F_r(:, indCol) = Ra_t * CaB_d ; % loads at the nodes
                %             F_rg{jj}(:,ind3) = Ra_t;
                
                %Loads from point masses
                %gravForce_pt                = ModelData(jj).MassData.M_pt_store(:, :, ii) * [CGB_e' zeros(3); zeros(3) CGB_e'] * Sim.grav_vec;
                %SYS(jj).f_grav_pt(indCol,1) = gravForce_pt;
                %SYS(jj).f_grav_pt_glob(ind3,1) = [CGB_e' zeros(3); zeros(3) CGB_e']*gravForce_pt;
            end
            
            %Define sparse matrices
            SYS(jj).Atot      = sparse(SYS(jj).Atot);
            SYS(jj).dQstiftot = sparse(SYS(jj).dQstiftot);
        ...
        for jj = 1:nPart
            ...
            for ii = 1:nElem
                ...
                if 0
                    ...
                    IkronVO     =  kron(eye(6),X.x_v_p1{jj}(indCol));
                    IkronMF     =  kron(eye(6),X.x_f_p1{jj}(indCol));
                ...
                Atot{jj}(indRow,indCol)       = Atot{jj}(indRow,indCol)       + A;
                Ctot{jj}(indRow,indCol)       = Ctot{jj}(indRow,indCol)       + C;
                Etot{jj}(indRow,indCol)       = Etot{jj}(indRow,indCol)       + E;
                
                Ctot{jj}(indCol+6,indCol)     = Ctot{jj}(indCol+6,indCol)     + C_pt;
                
                dQstiftot{jj}(indRow,indCol)  = dQstiftot{jj}(indRow,indCol)  + dQstif;
                dQgyrtot{jj}(indRow,indCol)   = dQgyrtot{jj}(indRow,indCol)   + dQgyr;
                dTgamtot{jj}(indRow,indCol)   = dTgamtot{jj}(indRow,indCol)   + dTgam;
                
                dQgyrtot{jj}(indCol+6,indCol) = dQgyrtot{jj}(indCol+6,indCol) + dQgyr_pt;
                
                pa    = X.x_v_p1{jj}(indCol(1:3));%PosOrientNodal(ind3(1:3));
                omega = X.x_v_p1{jj}(indCol(4:6));%PosOrientNodal(ind3(4:6));
                ...
                CGB_tot{jj}(ind4,indCol)  = [CGB_e,zeros(3)];
                ...
                if ~Sim.speed_up
                    ...
                    dOmegaQuat{jj}(ind1,indCol) = [zeros(4,3),dOmegaQuat_e];
                    ...
                    dOmegaQuat{jj}(ind1,indCol) = zeros(4,6);
                ...
                if Sim.rb_flag || Sim.rbloads_flag
                    ...
                    M_r{jj}(:,indCol)     = (Ra_t*      CaB_d*        Matrices.Aa2{ jj}(:,:,ii  ))*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii)) + ...
                        (Ra_t_node* CaB_d*        Matrices.M_pt{jj}(:,:,ii+1))* Matrices.T_pt{jj}(:,:,ii);
                    
                    Qgyr_r{jj}(:,indCol)  = (Ra_t*      CaB_d*xvtilde*Matrices.Aa2{      jj}(:,:,ii))*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii)) + ...
                        (Ra_t_node* CaB_d*        Matrices.C_pt_pre1{jj}(:,:,ii))*IkronVO;
                    
                    dQgyr_r{jj}(:,indCol) = -Ra_t*      CaB_d*xvtilde_var                            *(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii)) + ...
                        +Ra_t_node* CaB_d*        Matrices.C_pt_pre2{jj}(:,:,ii) *IkronVO;
                    
                    dV_dva{jj}(indCol,:)  = CaB_d'*Ra_t';
                    
                    Btot_r{jj}(:,indCol)  = Ra_t * CaB_d * (Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii));
                    ...
                    if ~Sim.speed_up
                        ...
                        dQgyr_r_dRa             = [(dRa_t1*CaB_d*xvtilde*Matrices.Aa2{jj}(:,:,ii))*X.x_v_p1{jj}(indCol)*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii)), ...
                            (dRa_t2*CaB_d*xvtilde*Matrices.Aa2{jj}(:,:,ii))*X.x_v_p1{jj}(indCol)*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii)), ...
                            (dRa_t3*CaB_d*xvtilde*Matrices.Aa2{jj}(:,:,ii))*X.x_v_p1{jj}(indCol)*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii))];
                        dQgyr_r_dp{jj}(:,ind4)  =               dQgyr_r_dRa*CGa';
                        dQgyr_r_dq{jj}(:,ind1)  = [(Ra_t*CaG_d*dCBG_e1_blkdiag'*xvtilde*Matrices.Aa2{jj}(:,:,ii))*X.x_v_p1{jj}(indCol)*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii)),...
                            (Ra_t*CaG_d*dCBG_e2_blkdiag'*xvtilde*Matrices.Aa2{jj}(:,:,ii))*X.x_v_p1{jj}(indCol)*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii)),...
                            (Ra_t*CaG_d*dCBG_e3_blkdiag'*xvtilde*Matrices.Aa2{jj}(:,:,ii))*X.x_v_p1{jj}(indCol)*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii)),...
                            (Ra_t*CaG_d*dCBG_e4_blkdiag'*xvtilde*Matrices.Aa2{jj}(:,:,ii))*X.x_v_p1{jj}(indCol)*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii))];
                        dQgyr_r_dpa             = dQgyr_r_dpa - dQgyr_r_dRa*CGa';
                        dQgyr_r_dqa             = dQgyr_r_dqa + dQgyr_r_dRa*dRa_dqa + ...
                            [(Ra_t*dCaG_e1_blkdiag*blkdiag(CGB_e,CGB_e)*xvtilde*Matrices.Aa2{jj}(:,:,ii))*X.x_v_p1{jj}(indCol)*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii)),...
                            (Ra_t*dCaG_e2_blkdiag*blkdiag(CGB_e,CGB_e)*xvtilde*Matrices.Aa2{jj}(:,:,ii))*X.x_v_p1{jj}(indCol)*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii)),...
                            (Ra_t*dCaG_e3_blkdiag*blkdiag(CGB_e,CGB_e)*xvtilde*Matrices.Aa2{jj}(:,:,ii))*X.x_v_p1{jj}(indCol)*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii)),...
                            (Ra_t*dCaG_e4_blkdiag*blkdiag(CGB_e,CGB_e)*xvtilde*Matrices.Aa2{jj}(:,:,ii))*X.x_v_p1{jj}(indCol)*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii))];
                    end
                end
                %% Aero
                if Sim.aero_flag && Matrices.isLS{jj}
                    ...
                    v_A0     = CA0B_tot_blkdiag*(X.x_v_p1{jj}(indCol) - [CGB_e' zeros(3); zeros(3) zeros(3)]*[gust_vec{jj}(ind4);0;0;0]);
                    %                 v_elem     = v_A0(1:3);
                    %                 omega_elem = v_A0(4:6);
                    %
                    %                 ydot = -v_elem(2);
                    ...
                    if 1%~Sim.speed_up
                        ...
                        dF_e(4,4) = -pi*Aero.rho*chord^3/8/2*ydot*tiplossFactor;
                        dF_tot{jj}(indCol,indCol) = CBA0_tot_blkdiag * dF_e * CA0B_tot_blkdiag;
                    else
                        dF_tot{jj}(ind3,ind3) = zeros(6);
                    end
                    
                    if ~Sim.speed_up
                        dF_wg_tot{jj}(indCol,ind4) = - CBA0_tot_blkdiag * dF_e * CA0B_tot_blkdiag * [CGB_e';zeros(3)];
                        dF_u_e                   =   zeros(6,1);
                        ...
                        dF_u_tot{jj}(indCol,ii)    =   CBA0_tot_blkdiag * dF_u_e;
                    else
                        dF_wg_tot{jj}(indCol,ind4) = zeros(6,3);
                        dF_u_tot{jj}(indCol,ii)    = zeros(6,1);
                    end
                    ...
                    dA_x_1_e(:,3)         =  - 1/ydot  *[1;1];
                    dA_x_1{jj}(ind5,indCol) = dA_x_1_e*CA0B_tot_blkdiag;
                    
                    if ~Sim.speed_up
                        dA_x_e = zeros(2,6);
                        dA_x_e(:,2) = -2/chord*diag([Aero.Leishman.b1 Aero.Leishman.b2])*X.x_x_p1{jj}(ind5);
                        dA_x{jj}(ind5,indCol)       =   dA_x_e  *CA0B_tot_blkdiag;
                    ...
                    dF_x{jj}(indCol,ind5) =  CBA0_tot_blkdiag * dF_x_e;%blkdiag(C_twist',C_twist')*dF_x_e;
                else
                    f{jj}(indCol,1)  = zeros(6,1);
                end
                %% Gravity
                if Sim.grav_fact
                    gravForce               = Matrices.Aa2{jj}(:,:,ii)*[CGB_e' zeros(3); zeros(3) CGB_e']*Sim.grav_vec;
                    f_grav{jj}(indCol,1)      = gravForce;
                    f_grav_glob{jj}(indCol,1) = [CGB_e' zeros(3); zeros(3) CGB_e']*gravForce;
                    
                    if ~Sim.speed_up
                        dF_grav_tot{jj}(indCol,ind1) = [Matrices.Aa2{jj}(:,:,ii)*dCBG_e1_blkdiag*Sim.grav_vec...
                        ...
                        dF_grav_tot{jj}(indCol,ind1) = zeros(6,4);
                    end
                else
                    f_grav{jj}(indCol,1)      = zeros(6,1);
                    if ~Sim.speed_up
                        dF_grav_tot{jj}(indCol,ind1)   = zeros(6,4);
                        if Sim.rb_flag
                ...
                if Sim.aero_flag && Matrices.isLS{jj} && ~Sim.speed_up && Sim.rb_flag
                    dF_tot_dq{jj}(:,ind1) = [(Ra_t*CaG_d*dCBG_e1_blkdiag'*f{jj}(indCol,1))*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii)),...
                        (Ra_t*CaG_d*dCBG_e2_blkdiag'*f{jj}(indCol,1))*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii)),...
                        (Ra_t*CaG_d*dCBG_e3_blkdiag'*f{jj}(indCol,1))*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii)),...
                        (Ra_t*CaG_d*dCBG_e4_blkdiag'*f{jj}(indCol,1))*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii))];
                    dF_tot_dRa            = [(dRa_t1*CaB_d*f{jj}(indCol,1))*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii)), ...
                        (dRa_t2*CaB_d*f{jj}(indCol,1))*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii)), ...
                        (dRa_t3*CaB_d*f{jj}(indCol,1))*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii))];
                    dF_tot_dp{jj}(:,ind4) =              dF_tot_dRa*CGa';
                    dF_tot_dpa            = dF_tot_dpa - dF_tot_dRa*CGa';
                    dF_tot_dqa            = dF_tot_dqa + dF_tot_dRa*dRa_dqa + ...
                        [(Ra_t*dCaG_e1_blkdiag*blkdiag(CGB_e,CGB_e)*f{jj}(indCol,1))*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii)),...
                        (Ra_t*dCaG_e2_blkdiag*blkdiag(CGB_e,CGB_e)*f{jj}(indCol,1))*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii)),...
                        (Ra_t*dCaG_e3_blkdiag*blkdiag(CGB_e,CGB_e)*f{jj}(indCol,1))*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii)),...
                        (Ra_t*dCaG_e4_blkdiag*blkdiag(CGB_e,CGB_e)*f{jj}(indCol,1))*(Matrices.s{jj}(ii+1)-Matrices.s{jj}(ii))];
                else
            ...
            for ii = 1:nNode
                ...
                if Sim.grav_fact
                    ...
                    f_grav_pt{jj}(indCol,1)      = gravForce_pt;
                    f_grav_pt_glob{jj}(indCol,1) = [CGB_e' zeros(3); zeros(3) CGB_e']*gravForce_pt;
                    ...
                    if ~Sim.speed_up && ii > 1 && Sim.rb_flag
                        ...
                        dF_grav_pt_tot{jj}(indCol,ind1) = [Matrices.M_pt{jj}(:,:,ii)*dCBG_e1_blkdiag*Sim.grav_vec...
                        ...
                        dF_grav_pt_tot{jj}(indCol,ind1) = zeros(6,4);
                    end
                else
                    f_grav_pt{jj}(indCol,1)      = zeros(6,1);
                    if ~Sim.speed_up
                        dF_grav_pt_tot{jj}(indCol,ind1)   = zeros(6,4);
                        if Sim.rb_flag
                ...
                f_glob{jj}(indCol,1)         = [CGa',zeros(3);zeros(3),CGa']*Matrices.f_a{jj}(indCol);
            end
        end
...
function SYS = initialiseStaticSysStructure(nNode, nElem)
%initialiseStaticSysStructure Sets up the structure array for the 'SYS'
%data structure that is returned by the function 'getSystemMatrices'.

%Preallocate
%   - System global stiffness matrix (e.g. [Atot] * {X} - {f} = 0)
Atot      = arrayfun(@(i) zeros(nNode(i) * 6, nElem(i) * 6), 1 : numel(nNode), 'Unif', false);
%   - Derivative of system w.r.t local loads (states)
%   e.g. dQstiftot = d[Atot]/d{X}
dQstiftot = arrayfun(@(i) zeros(nNode(i) * 6, nElem(i) * 6), 1 : numel(nNode), 'Unif', false);
%   - Loads
f           = arrayfun(@(nE) zeros(nE * 6, 1), nElem, 'Unif', false);
f_grav      = arrayfun(@(nE) zeros(nE * 6, 1), nElem, 'Unif', false);
f_grav_glob = arrayfun(@(nE) zeros(nE * 6, 1), nElem, 'Unif', false);
%AoA_qs_t    = arrayfun(@(nE) zeros(nE, 1)    , nElem, 'Unif', false);
Btot_r      = arrayfun(@(nE) zeros(6, nE * 6), nElem, 'Unif', false);
f_glob      = arrayfun(@(nN) zeros(nN * 6, 1), nNode, 'Unif', false);
f_grav_pt   = arrayfun(@(nN) zeros(nN * 6, 1), nNode, 'Unif', false);
F_r         = arrayfun(@(nN) zeros(6, nN * 6), nNode, 'Unif', false);

%Define as structure array
SYS = struct('Atot', Atot, 'dQstiftot', dQstiftot, 'f', f, 'f_grav', f_grav, ...
    'f_grav_glob', f_grav_glob, 'Btot_r', Btot_r, 'f_glob', f_glob, ...
    'f_grav_pt', f_grav_pt, 'F_r', F_r);
