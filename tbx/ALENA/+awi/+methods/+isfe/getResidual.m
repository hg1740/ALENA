function [r_full, Q ,S_sparse, D_scale] = getResidual(ModelData, System, AnalysisParam, bUpdateTangent, SYS, tt)
...
switch AnalysisParam.AnalysisType
    ...
    case 'static'
        ...
        for jj = 1 : nPart
            ...
            %Indexing into the state vector (forces and moments) 
            indQ = System.BodyLocalLoadIndex(1, jj) :  System.BodyLocalLoadIndex(2, jj);
            q    = System.x_f(indQ);
            
            %Grab part data
            %   - Orientation vector            
            e1tot    = ModelData(jj).E1(:);
            %   - FE derivative matrix
            Dtot     = ModelData(jj).ShapeFunctions.Derivative;
            %   - FE integration matrix
            Btot_mp  = ModelData(jj).ShapeFunctions.Integral; 
            %   - Cantilever boundary conditions
            R1_bc    = ModelData(jj).BoundaryConditions.R1_bc;
            ...
            %       Dtot * {F ; M} = {F' ; M'}
            Qstif{jj} = (SYS(jj).Atot * (q + e1tot)) - (Dtot * q);
            ...
            % * * r_f is the residual * *
            % - ModelData.f_n are follower loads
            ...
            r_f{jj} = r_f{jj} + R1_bc * ( ...
                Qstif{jj} - ModelData(jj).f_n - SYS(jj).f_glob - SYS(jj).f_grav_pt - ...
                Btot_mp * (SYS(jj).f + SYS(jj).f_grav));
            
            %Update the tangent matrix
            if bUpdateTangent
                S_full(indQ, indQ) = R1_bc * (SYS(jj).Atot - Dtot - SYS(jj).dQstiftot);
