classdef ScalarMass < awi.fe.FEBaseClass
    %ScalarMass Describes a generic spring element connecting two nodes.
    %
    % The definition of the 'ScalarMass' object matches that of the
    % CMASS1 bulk data type from MSC.Nastran.
    
    %Primary Properties
    properties
        %ScalarMass element identification number
        EID
        %ScalarMass property idenfitication number
        PID
    ...
    properties (Hidden = true)
        %Handle to the 'awi.fe.Node. object that this 'ScalarMass' connects to
        Nodes          = [];
        %Handle to the 'awi.fe.ScalarMassProp' object
        ScalarMassProperty = [];
    end
    ...
    methods % set / get
        ...
        function set.ScalarMassProperty(obj, val) %set.ScalarMassProperty 
            %set.ScalarMassProperty Set method for the property 'ScalarMassProperty'
            %
            % 'ScalarMassProperty' must be a scalar 'awi.fe.ScalarMassProp' object.
            
            validateattributes(val, {'awi.fe.ScalarMassProp'}, { ...
                'scalar', 'nonempty'}, class(obj), 'ScalarMassProperty');
            obj.ScalarMassProperty = val;
        end
        ...
        function val = get.PID(obj)           %get.PID            
            ...
            % 'awi.fe.ScalarMassProp' object then always use their ID number, 
            % else use PID.
            
            if isempty(obj.ScalarMassProperty)
                val = obj.PID;
            else
                val = obj.ScalarMassProperty.ID;                
            end
        end
    end
    
    methods % construction
        function obj = ScalarMass
    ...
    methods % writing data in MSC.Nastran format
        function writeToFile(obj, fid, bComment)
            %writeToFile Write the data for the 'awi.fe.ScalarMass' object into
            %a text file using the format of the MSC.Nastran CMASS1 bulk
            %data entry. 
            ...
            if bComment %Helpful comments?
                comment = 'CMASS1 : Defines a scalar mass element';
